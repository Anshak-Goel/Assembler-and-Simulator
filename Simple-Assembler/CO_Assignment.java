        ...
        for (int i=0;i<input.size();i++){
            ...
            if(input.get(i).get(0).equals("instruction")){
                ...
                if(label_instruction.size()==1){
                    ...
                    ret_value=false;
                    continue;
                }else if(input.get(i).size()>2 && input.get(i).get(2).substring(input.get(i).get(2).length() - 1).equals(":")) {
                	System.out.println("Error in Line "+(i+1)+" : Can't define multiple labels on same line");
                        ret_value=false;
                	continue;
                }
